...
sub show_collector_records { my ($HTCONDOR_COLLECTOR_HOST, $title, $as_table) = @_ ;
	if (! $HTCONDOR_COLLECTOR_HOST) {
		$log->error("HTCondor Collector Host not defined");
		return;
	}
	...
	if ($title eq 'assessment') {
		...
		$log->error("Error - $title incorrectly specified");
		return;
	}
	...
	if ($status) {
		$log->error("Error - condor_status $HTCONDOR_COLLECTOR_HOST show $title records failed - status: $status output: $output");
		return;
	}
	my @lines = split "\n", $output;
