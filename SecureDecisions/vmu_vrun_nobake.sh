...
# start in $JOB_INPUT_DIR - the viewer VM input directory
cd $JOB_INPUT_DIR
...
cp $JOB_INPUT_DIR/swamp-shutdown-service /etc/init.d/.
...
chown -R mysql:mysql /var/lib/mysql/* >> $SWAMP_LOG_FILE 2>&1
...
service mysql start >> $SWAMP_LOG_FILE 2>&1
if [ $? -ne 0 ]
then
	record_event MYSQLFAIL "Service mysql failed to start"
	service mysql status >> $SWAMP_LOG_FILE 2>&1
	if [ $shutdown_on_error -eq 1 ]
	then
...
cp ${viewer}.props /var/lib/${viewer}/$PROJECT/config >> $SWAMP_LOG_FILE 2>&1
cp logback.xml /var/lib/${viewer}/$PROJECT/config >> $SWAMP_LOG_FILE 2>&1
...
chown -R tomcat:tomcat $TOMCATDIR >> $SWAMP_LOG_FILE 2>&1
...
service $TOMCATSERVICE start >> $SWAMP_LOG_FILE 2>&1
tomcat_started=0
for i in {1..10}
do
...
then
	...
	service $TOMCATSERVICE status >> $SWAMP_LOG_FILE 2>&1
	if [ $shutdown_on_error -eq 1 ]
	then
...
echo "*/$CHECKTIMEOUT_FREQUENCY * * * * root $JOB_INPUT_DIR/checktimeout" >> /etc/crontab
