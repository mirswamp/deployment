...
sub show_condor_queue {
	my $command = "condor_q";
	my ($output, $status) = systemcall($command);
	if ($status) {
		print "Error - condor_q failed - status: $status output: $output\n";
		return;
	}
	if (! $output) {
...
sub show_virtual_machines {
	my $command = "sudo virsh list --all";
	my ($output, $status) = systemcall($command);
	if ($status) {
		print "Error - virsh list --all failed - status: $status output: $output\n";
		return;
	}
	if (! $output) {
		print "no virsh records\n";
		return;
	}
	print $output, "\n";
}
