...
sub swamponabox_setup {
	print "\nSwampOnABox Database Setup\n";
	foreach my $file (@swamponabox_setup_files) {
		my $script = File::Spec->catfile($root, $file);
		run_sql_script($script);
	}
}

sub swamponabox_upgrade{
	print "\nSwampOnABox Database Upgrade\n";
	...
	chdir $cwd;
	foreach my $file (@swamponabox_upgrade_files) {
		my $script = File::Spec->catfile($root, $file);
		run_sql_script($script);
	}
}
