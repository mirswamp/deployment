...
patch_value('/opt/swamp/etc/swamp.conf', '^dbQuartermasterPass\s*=\s*(.*)', $javapassword);
...
chomp $webpassword;
if ($patch_token) {
	print "Patching swamp-web-server WEBPASSWORD\n";
	patch_token('/var/www/swamp-web-server/.env', 'WEBPASSWORD', $webpassword);
}
else {
	print "Patching swamp-web-server *DB_PASSWORD\n";
	patch_value('/var/www/swamp-web-server/.env', '^.*DB_PASSWORD\s*=\s*(.*)', $webpassword);
}
