...
RELEASE_ROOT="$WORKSPACE/export/swamponabox/distribution"
...
function build_rpms() {
	...
	$BINDIR/../singleserver/sbin/jenkins_build_rpms.bash swamponabox $WORKSPACE $RELEASE_NUMBER $BUILD_NUMBER
}
...
function build_src_tar() {
	...
	tar -C $(dirname $BUILD_RESULT) -cvzf $(dirname $BUILD_RESULT)/build_swampinabox-src.tar.gz $(basename $BUILD_RESULT) sample_packages repos
}
