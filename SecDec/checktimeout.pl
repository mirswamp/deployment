...
my $tomcatlog = '/var/log/tomcat6/catalina.out';
...
sub find_vm_start_timestamp {
	if (open(my $fh, '<', $swamplog)) {
		...
		foreach my $line (@lines) {
			if ($line =~ m/^(.*) Starting CodeDX viewer via run.sh$/) {
				my $date = $1;
				last if (! $date);
...
my $CHECKTIMEOUT_LASTLOG = $ARGV[0] || 3600;
my $epoch_now = time();
...
system("echo CodeDX Running time: $runtime_seconds seconds >> $swamplog");
...
system("echo CodeDX Time since last log: $lastlog_seconds seconds >> $swamplog");
